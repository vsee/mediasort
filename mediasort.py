...
def batch_convert_heic_to_jpeg(source_dir):
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        future_to_file = {
            executor.submit(convert_heic_to_jpeg, heic_path, jpeg_path): (
                heic_path,
                jpeg_path,
            )
            for heic_path, jpeg_path in heic_files
        }
...
class MediaType(Enum):
    ...
    def filename_to_mediaType(cls, filename):
        ...
        if extension in VIDEOTYPES:
            ...
            print(
                "WARNING unhandled media extension ["
                + extension
                + "] for file "
                + filename
            )
...
class MediaFile:
    ...
    def __repr__(self):
        dateStr = (
            time.strftime("%Y-%m-%d", self.dateTaken)
            if self.dateTaken is not None
            else "NONE"
        )
...
def getImageDateTaken(src):
    with open(src, "rb") as imgFile:
        imgTags = exifread.process_file(
            imgFile, details=False, stop_tag="EXIF DateTimeOriginal"
        )
...
def getDateTaken(mfile):
    ...
    if mfile.prefix is None:
        print("DEBUG: " + mfile.srcFile)

    if date is None:
        print(
            "WARNING: date not extracted for "
            + mfile.srcFile
            + ". Using Creation Timestamp."
        )
...
def addToCollection(mediafiles, mfile):
    ...
    if mfile.mediaType not in mediafiles:
        if mfile.mediaType == MediaType.UNKNOWN:
            mediafiles[mfile.mediaType] = []
        else:
            mediafiles[mfile.mediaType] = {}
...
def sortFiles(outDir, mediafiles, custom_prefix):
    ...
    for mtype, years in mediafiles.items():
        if mtype == MediaType.UNKNOWN:
            ...
            sortedDir = os.path.join(outDir, "sorted", "videos")
        elif mtype == MediaType.AUDIO:
            sortedDir = os.path.join(outDir, "sorted", "audio")
        elif mtype == MediaType.IMAGE:
            sortedDir = os.path.join(outDir, "sorted", "images")
        else:
            raise RuntimeError("Unhandled media type: " + mtype)
